...
* REMAKE OF ULIN GANS × BRYAN GAY:V
...
const { help } = require('./ULIN/help')
const { rules } = require('./ULIN/rules')
const { bahasa } = require('./ULIN/bahasa')
const { donasi } = require('./ULIN/donasi')
const { limits } = require('./ULIN/limit')
const { bokep } = require('./ULIN/bokep.js')
const { ownermenu } = require('./ULIN/ownermenu.js')
const { groupmenu } = require('./ULIN/groupmenu.js')
const { mediamenu } = require('./ULIN/mediamenu.js')
const { nsfwmenu } = require('./ULIN/nsfwmenu.js')
const { listmenu } = require('./ULIN/listmenu.js')
const { funmenu } = require('./ULIN/funmenu.js')
const { makermenu } = require('./ULIN/makermenu.js')
//jangan hiraukan ini!!,,,ini uji coba yang gagal:) kontol//
//const { gabutmenu } = require('./ULIN/gabutmenu')
//const { ownermenu } = require('./ULIN/ownermenu')
//const { groupmenu } = require('./ULIN/groupmenu')
//const { vipmenu } = require('./ULIN/vipmenu')
//const { nsfwmenu } = require('./ULIN/nsfwmenu')
//const { creatormenu } = require('./ULIN/creatormenu')
//const { downloadmenu } = require('./ULIN/downloadmenu')
//const { allmenu } = require('./ULIN/allmenu')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:ULIN GANS\n'//GANTI NAMA LU COK
            + 'ORG:Creator ULINBot;\n'//GANTI NAMA LU!!
            + 'TEL;type=CELL;type=VOICE;waid=6287732554856:+62 877-3255-4856\n'//GANTI NOMOR LU
            + 'END:VCARD'
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('BURUAN SCAN!! Subscribe ULIN GANS'))
	})

	fs.existsSync('./ULIN.json') && client.loadAuthInfo('./ULIN.json')
	...
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./ULIN.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ULINGANS = ["6287732554856@s.whatsapp.net"]
			...
			const ownerNumber = [
			"6287732554856@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"6283838897315@s.whatsapp.net","6283102650464@s.whatsapp.net"//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
     ...
     const bayarLimit = (sender, amount) => {
		...
		case 'daftar':
			...
			case 'ULIN':
                ...
                setTimeout( () => {
					costum(help(prefix, instagram, yt, name, pushname2, user, limitt), text, ULINGANS, replySet)
					}, 1500)
					setTimeout( () => {
					/*client.sendMessage(from, rules(name, uptime, tanggal, jam, prefix), text, {quoted: mek })
					}, 1200)
					setTimeout( () => {*/
					client.sendMessage(from, 'Terimakasih sudah menggunakan *ULIN Bot* \n\n*SEBAGAI PENGGANTI DONASI YUK SUPORT*\nSubscribe https://youtube.com/c/ULINGANS \nFollow instagram https://instagram.com/ahmd.fdhl_', text, {quoted: mek})
					}, 0)
    				break
/*********PISAH MENU**********/
              case 'ownermenu':
		    ...
		    case 'map':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *ULINBOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak *${pushname2}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu!*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ULINBOT]','Yellow'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'yellow'))
